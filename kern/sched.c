...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	ready_queues = kmalloc(num_of_ready_queues * sizeof(struct Env_Queue));
	for(uint8 i = 0; i < num_of_ready_queues; i++)
	{
...
struct Env* fos_scheduler_MLFQ()
{
	if(curenv != NULL)
	{
		for(uint8 i = 0 ; i < num_of_ready_queues ; i++)
		{
			if (queue_size(&(ready_queues[i])) > 0)
			{
				struct Env* ff = find_env_in_queue(&(ready_queues[i]),curenv->env_id);
				if (ff != NULL)
				{
					remove_from_queue(&(ready_queues[i]),curenv);
	...
	for(uint8 i = 0 ; i < num_of_ready_queues ; i++)
	{
		if (queue_size(&(env_ready_queues[i])) > 0)
		{
			...
			uint8 z =  i == num_of_ready_queues-1 ? num_of_ready_queues-1 : i+1;
			enqueue(&(ready_queues[z]),nextenv);
