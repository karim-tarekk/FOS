...
{
	uint32* PT_ptr = kmalloc(PAGE_SIZE);
	memset((uint32*)PT_ptr , 0, PAGE_SIZE);
	uint32* pa = &ptr_page_directory[PDX(virtual_address)];
	uint32* PT_ptr_PA = (uint32*)kheap_physical_address((uint32)PT_ptr);
	*pa = ((uint32)PT_ptr_PA|PERM_USER|PERM_PRESENT| PERM_WRITEABLE);
	tlbflush();
	return (void*)PT_ptr;
}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int WSmaxSize = e->page_WS_max_size;
	for(int i=0; i< WSmaxSize; i++)
	{
		uint32 WSVA = env_page_ws_get_virtual_address(e,i);
		if((WSVA >= address) && (WSVA < (address + ROUNDUP(size,PAGE_SIZE))))
		{
	...
	for(int i=0; i<Pages; i++)
	{
		int f = 1;
		PT_ptr=NULL;
		get_page_table(e->env_page_directory,(void*)address,&PT_ptr);
		if(PT_ptr !=NULL)
		{
			...
			{
				kfree((void*)PT_ptr);
				pd_clear_page_dir_entry(e,(uint32)address);
			}
		}
		address += PAGE_SIZE;
	}
