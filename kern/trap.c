...
void plc(struct Env * curenv, uint32 fault_va)
{
	struct Frame_Info *NewFrame = NULL;
	int res = allocate_frame(&NewFrame);
	if(res != E_NO_MEM)
	{
		map_frame(curenv->env_page_directory ,NewFrame ,(void*)fault_va, PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
		...
		if (res == E_PAGE_NOT_EXIST_IN_PF)
		{
			  if (fault_va < USTACKTOP && fault_va >= USTACKBOTTOM)
				  ...
				  panic("Error Can Not Access %x",fault_va);
		}

		int WSsize = curenv->page_WS_max_size ;
		for( int i =0 ; i < WSsize;i++)
		{
			if(env_page_ws_is_entry_empty(curenv,curenv->page_last_WS_index))
				break;
			else if(env_page_ws_is_entry_empty(curenv,i))
			{
...
uint32 FindVictimVA(struct Env * curenv)
{
	uint32 victimVA = 0;
	int cycle = -1;
	int last = curenv->page_WS_max_size -1;
	int c;
	c = curenv->page_last_WS_index;
	while (c < curenv->page_WS_max_size)
	{
		uint32 va = env_page_ws_get_virtual_address(curenv,c);
		uint32 perm = pt_get_page_permissions(curenv,va);
		int accessed = perm & PERM_USED;
		if (!accessed)
		{
			victimVA = va;
			return victimVA;
		...
		else
		{
			...
			else if(c == curenv->page_last_WS_index && cycle==1)
				break;
			c++;
		}
	}
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	int WSmaxSize = curenv->page_WS_max_size ;
	...
	else
	{
		...
		if (isPageReplacmentAlgorithmModifiedCLOCK())
			add = FindVictimVA(curenv);
		else
			add = env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
		uint32 *PT_ptr = NULL ;
		struct Frame_Info *frame_info_ptr = get_frame_info(curenv->env_page_directory,(void *)add, &PT_ptr);
		uint32 per = pt_get_page_permissions(curenv,add);
		if((per & PERM_MODIFIED) == PERM_MODIFIED)
		{
			int out = pf_update_env_page(curenv,(void*)add,frame_info_ptr) ;
		}
		unmap_frame(curenv->env_page_directory,(void*) add);
		env_page_ws_invalidate(curenv,add);
		plc(curenv, fault_va);
	}
}
